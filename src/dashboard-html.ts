...
function onVoiceChange(sel) {
  ...
  // Update the description text from the catalog to match the new voice
  var catalogEntry = window.__voicesState.catalog.find(function(v) { return v.name === newVoice; });
  if (catalogEntry) {
    row.name = newVoice + ' / ' + catalogEntry.style;
  }
...
async function saveVoices(applyAfter) {
  ...
  const updates = rows
    ...
    .map(function(r) { return { agent: r.agent, gemini_voice: r.gemini_voice, name: r.name || '' }; });
